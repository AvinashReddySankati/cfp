...
DATABASE="/Users/avinashreddysankati/Documents/uc/4.CloudComputing/FProject/initial.db"
app = Flask(__name__)
...
def viewDashboard():
    #PieGraph
    #over (partition by q.HSHD_NUM, q.BASKET_NUM)
    data=exeselect("Select count(q.PURCHASE_) as Expense,p.HH_SIZE as Household_Size from households as p inner join transactions as q on p.HSHD_NUM=q.HSHD_NUM and p.HH_SIZE is not null group by p.HH_SIZE;");
    ...
    return render_template("viewDashboard.html",title='Expense(Avg) vs Household Size ', max=17000,titletwo="Expense(Avg) vs Age_Group vs Organic Products",
                           labels=data['Household_Size'].values.tolist(), values=data['Expense'].values.tolist(),
